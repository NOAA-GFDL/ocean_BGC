...
module generic_ERGOM
  ...
  type generic_ERGOM_type
     ...
     ! 
     real :: &
          q10_nit    = 0., &   ! q10 parameter for nitrification [1/°C]
          q10_h2s    = 0., &   ! q10 parameter for chemolithotrophs (so4 reduction) [1/°C]
  ...
  type zooplankton
     ...
     real :: &
	  pnr            = 0. , &  ! P/N ratio
	  cnr            = 0. , &  ! C/N ratio
	  t_opt          = 0. , &  ! optimal grazing temperature [°C]
          t_max          = 0. , &  ! maximal grazing temperature [°C]
  ...
  type detritus
     ...
     real :: &
	  ...
	  q10_rec  = 0.	          ! q10 parameter for recycling of detritus [1/�C]
     integer ::        &
  ...
  type sediment
     ...
     real :: &
	  ...
	  q10_rec         = 0. , & ! q10 parameter for recycling [1/�C]
	  ...
	  wsed            = 0. , & ! sedimentation rate [m/s]
          ...
          po4_ret_plus_BB = 0. , & ! value added to po4_retention north of 60.75�N 
...
! Most ecosystem parameters use 1/d as time unit. This is to convert  
  ...
  real, dimension(maxphyt) :: &
        ...
        tmin   , &	    ! minimum phytoplankton growth temperature (for cyanobacteria only) [°C]
  ...
  real, dimension(maxzoo)  :: &
        t_opt_zoo  , &      ! temperature optimum of grazing [°C]
        t_max_zoo  , &      ! maximal grazing temperature [°C]
  ...
  real, dimension(maxdet)   :: &
        dn     , &          ! recycling rate
	q10_rec             ! q10 parameter for recycling [1/�C]
  character(len=32)         :: &
        name_det(maxdet), &
  ...
  real  :: &
        ...
        q10_nit   = .11      , &  ! q10 parameter for nitrification [1/°C]
        alpha_nit = 3.75e-6  , &  ! half-saturation constant for nitrification [mol/kg]
        q10_h2s   = 0.0693   , &  ! q10 parameter for chemolithotrophs (h2s oxidation) [1/°C]
  ...
  data (t_opt_zoo(n),   n=1, maxzoo)  /15.0, 10.0, 10.0, 10.0/	 ! temperature optimum of zooplankton for grazing [°C]
  data (t_max_zoo(n),   n=1, maxzoo)  /25.0, 15.0, 15.0, 15.0/	 ! maximal grazing temperature [°C]
  ...
  data (q10_rec(n),  n=1, maxdet)  /0.0693, 0.0693/	  ! q10 paramter for recycling of detritus [1/°C]
...
! zooplankton parameters
   ...
   wo2          , & ! weight number for o2 sensitivity
   wh2s         , & ! weight number for h2s sensitivity
   t_opt_zoo    , & ! temperature optimum of zooplankton for grazing [°C]
   t_max_zoo    , & ! temperature maximum of zooplankton for grazing [°C]
   beta_zoo     , & ! parameter for temperature dependence of grazing [dimensionless]
   oxy_sub_zoo ,  & ! oxygen level below which reduced respiration starts [mol/kg]
   oxy_min_zoo,   & ! oxygen level below which no respiration takes place [mol/kg]
   resp_red_zoo,  & ! reduction factor for respiration under suboxic conditions [dimensionless]
...
! detritus parameters
   ...
   q10_rec      , & ! q10 paramter for recycling of detritus [1/°C]
! generic_ERGOM_type parameters
   nf	        , & ! nitrification rate [1/d]
   q10_nit      , & ! q10 parameter for nitrification [1/°C]
   alpha_nit    , & ! half-saturation constant for nitrification [mol/kg]
   q10_h2s      , & ! q10 parameter for chemolithotrophs (h2s oxidation) [1/°C]
...
! sediment parameters
   ...
   q10_rec_sed  , & ! q10 paramter for recycling of detritus in the sediment [1/°C]
   ...
   po4_ret_plus_BB, &! value added to po4_retention north of 60.75°N 
                     !(special treatment for the Bothnian Bay to supress cyanobacterial blooms)
...
! Oxic and anoxic detritus recycling (denitrification, anammox)
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  
      ...
      ! i.e. only between 7 and 30°C (Dalsgaard & Thamdrup 2002 in ApplEnvMicrobiol) 
